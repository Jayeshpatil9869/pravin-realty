...
export function PropertyDetail({ onOpenConsultation }: PropertyDetailProps) {
  ...
  // Specific high-res 4-grid photos for interior showcase
    ...
    <div className="min-h-screen pt-24 sm:pt-28 md:pt-36 space-y-12 sm:space-y-16 md:space-y-24 text-left overflow-x-hidden">
      ...
      <section className="px-4 sm:px-6 md:px-8 max-w-7xl mx-auto">
        ...
        <div className="space-y-5 sm:space-y-6">
          ...
          <ScrollReveal variant="blur-up" className="space-y-2.5 sm:space-y-3">
            ...
            <p className="flex items-center gap-1.5 text-neutral-500 text-xs sm:text-sm md:text-base font-normal">
      ...
      <section className="px-4 sm:px-6 md:px-8 max-w-7xl mx-auto space-y-8 sm:space-y-12">
        ...
        <ScrollReveal variant="fade-up" className="space-y-4 sm:space-y-6 max-w-3xl">
          ...
          <div className="space-y-3 sm:space-y-4">
            {neighborhoodTimes.map((item, idx) => (
              <div 
                key={idx} 
                className="flex items-center justify-between py-2 sm:py-2.5 border-b border-dashed border-neutral-200 text-xs sm:text-sm font-normal gap-4"
              >
                <span className="text-neutral-800">{item.name}</span>
                <span className="text-neutral-500 font-medium shrink-0">{item.time}</span>
              </div>
            ))}
          </div>
        </ScrollReveal>
        ...
        <ScrollReveal variant="scale-up" duration={0.7} className="space-y-4 sm:space-y-5">
          ...
          <div className="relative min-h-[220px] aspect-[21/10] sm:aspect-[2.6/1] w-full rounded-3xl overflow-hidden border border-neutral-200 shadow-sm bg-[#E9EBEF] flex items-center justify-center p-4 sm:p-6 text-center">
            ...
            <svg className="absolute inset-0 w-full h-full opacity-40 text-neutral-400 stroke-current pointer-events-none" fill="none">
            ...
            <div className="relative z-10 space-y-2.5 sm:space-y-3 px-2">
              <div className="inline-flex items-center gap-2 bg-white/95 backdrop-blur-md px-3.5 sm:px-5 py-2 sm:py-2.5 rounded-full shadow-lg border border-neutral-200 max-w-full">
                <span className="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full bg-[#9A3412] animate-ping shrink-0" />
                <span className="font-normal text-xs sm:text-sm md:text-base text-[#121316] truncate">
                  {property.title} • {property.neighborhood}, Pune
                </span>
              </div>

              <div className="flex flex-wrap items-center justify-center gap-1.5 sm:gap-3 text-[10px] sm:text-xs text-neutral-500 font-normal">
                <span className="bg-white/80 px-2.5 sm:px-3 py-1 rounded-full border border-neutral-200/60">Balewadi High Street (4 mins)</span>
                <span className="bg-white/80 px-2.5 sm:px-3 py-1 rounded-full border border-neutral-200/60">Mumbai-Pune Exp (8 mins)</span>
                <span className="bg-white/80 px-2.5 sm:px-3 py-1 rounded-full border border-neutral-200/60">Hinjewadi IT Park (12 mins)</span>
              </div>
            </div>
          </div>
        </ScrollReveal>
      ...
      <section className="px-3 sm:px-4 md:px-8 max-w-7xl mx-auto">
        <ScrollReveal variant="fade-up" duration={0.7} className="bg-white rounded-3xl md:rounded-[36px] p-5 sm:p-8 md:p-10 border border-neutral-200/80 shadow-[0_4px_24px_rgba(0,0,0,0.03)] space-y-5 sm:space-y-6 text-left">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-5 sm:gap-6 items-center pt-1 sm:pt-2">
            
            {/* Agent Profile & Contact */}
            <div className="md:col-span-8 flex items-center gap-3.5 sm:gap-5">
              <div className="w-14 h-14 sm:w-20 sm:h-20 rounded-2xl overflow-hidden border border-neutral-200 shadow-sm shrink-0">
                <img 
              ...
              <div className="min-w-0">
                <h4 className="text-base sm:text-xl font-normal text-[#121316] truncate">
                  {property.agent.name}
                </h4>
                <p className="text-xs sm:text-sm text-neutral-500 font-normal truncate">
                  {property.agent.role}
                </p>
                <p className="text-[11px] sm:text-xs text-neutral-400 font-normal mt-0.5 sm:mt-1 truncate">
            ...
            {/* Action Buttons */}
            <div className="md:col-span-4 flex flex-col sm:flex-row md:flex-col gap-2.5 w-full">
              <button
          ...
          {/* Certified Guarantee */}
          <div className="pt-3.5 sm:pt-4 border-t border-neutral-100 flex items-center gap-2.5 text-[11px] sm:text-xs text-neutral-500 font-normal leading-relaxed">
